...
const functions = {
    ...
    getUserRights: async (boardId, user) => {
        const { Members } = await boards.findOne(getSearch(boardId)).select('Members')
    ...
    teamExists: (name) => new Promise(resolve => {
        teams.find(getSearch(name), (err, data) => !data || data.length === 0 ? resolve(false) : resolve(true))
    }),

    getTeamMembers: (name) => new Promise(resolve => { 
        teams.find(getSearch(name), (err, data) => !data || data.length === 0 ? resolve([]) : resolve(data[0].Members))
    }),

    getTeamBoards: (name) => new Promise(resolve => {
        teams.find(getSearch(name), (err, data) => !data || data.length === 0 ? resolve([]) : resolve(data[0].Boards))
    }),

    boardExists: (name) => new Promise(resolve => {
        boards.find(getSearch(name), (err, data) => !data || data.length === 0 ? resolve(false) : resolve(true))
    }),

    getBoardMembers: (name) => new Promise(resolve => {
        boards.find(getSearch(name), (err, data) => !data || data.length === 0 ? resolve([]) : resolve(data[0].Members))
    }),
